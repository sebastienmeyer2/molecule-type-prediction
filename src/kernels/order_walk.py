"""Define the order walk graph kernel."""
...
class OrderWalkKernel(Kernel):
    """Order walk graph kernel."""
    ...
    def kernel(self, X: List[Graph], Y: Optional[List[Graph]] = None) -> np.ndarray:
        ...
        for Xi, Yi in zip(X_wl, Y_wl):
            ...
            # Progress bar
            if self.verbose:
                pbar = tqdm(range(n), desc=f"Computing {self.order}-th order walk kernel")
            else:
                pbar = range(n)
            ...
            # Compute walks
            X_walks = find_nodes_walks(Xi, order=self.order)
            if Y is not None:
                Y_walks = find_nodes_walks(Yi, order=self.order)
            else:
                Y_walks = X_walks
            ...
            for i in pbar:
                for j in range(m):
                    ...
                    if (j, i) in visited_graphs:
                        ...
                        K[i, j] += len(X_sequences[i] & Y_sequences[j])
